...
class UserManager(BaseUserManager):
    ...
    def create_user(self,username=None, phone_number=None, email=None, password=None, **extra_fields):
        if username is None:
            ...
            while User.obejects.filter(username=username).exists():
...
class User(AbstractBaseUser, PermissionsMixin):
    """
    An abstract base class implementing fully featured User model 
    with admin-compliant permission 
    ...
    phone_number = models.BigIntegerField(_('mobile number'), unique=True, null=True, blank=True,
        # validators=[validators.RegexValidator(r'^001[0-3,4]$',
        # ('Enter a vaalid mobile number.'))],
        error_messages={'unique':_('A user with this mobile already exists.')})
    ...
    def get_short_name(self):
        """
        Returns the short name for user
        """
    ...
    def is_loggedin_user(self):
        """
        Return True if user has actually logged in with valid credentails
        """
...
class UserProfile(models.Model):
    ...
    gender = models.BooleanField(_('gender'), help_text=_('femele is false, male is True, null is unset'), null=True)
    province  = models.ForeignKey(verbose_name=_('provience'), to='Province', null=True, on_delete=models.SET_DEFAULT, default='QC')
...
class Device(models.Model):
    ...
    deviceType = models.PositiveSmallIntegerField(choices=DEVICE_TYPE_CHOICES,default=WEB)
    deviceOS = models.CharField(_('device os'), max_length=20, blank=True)
    deviceModel = models.CharField(_('device model'), max_length=50, blank=True)
    appVersion = models.CharField(_('app version'), max_length=20, blank=True)
